...
function UserBook({
  ...
  const sendFriendRequestHandler = (receiverid) => {
    ...
    axios
      .post(`http://localhost:6600/friendrequest/sendrequest`,Users,{headers : { authorization : token}})
      .then((result) => {
        console.log("friendRequest_Sent");
      })
      .catch((error) => {
        console.log("FriendRequest_not_sent", error);
      });
  };
  ...
  return (
    <Box className="mainAppClass">
      ...
      <Box className="flex-container">
        ...
        <Box className="three">
          <Activity users={users} user={user} sendFriendRequestHandler={sendFriendRequestHandler}/>
        </Box>
      </Box>
    </Box>
  );
