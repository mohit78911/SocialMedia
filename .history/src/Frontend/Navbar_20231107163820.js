...
function Navbar({ requestData, getRequestHandler }) {
  ...
  const getAllRequestDataHandler = () => {
    axios
      .get("http://localhost:6600/friendrequest/allrequest",{headers:{authorization : token}})
      .then((result) => {
  ...
  const acceptFriendRequestHandler = () => {
    axios
      .delete(
        `http://localhost:6600/friendrequest/requestacceptandadd/${user._id}`
      )
      .then((result) => {
  ...
  const requestStatusHandler = (id) => {
    axios
      .put(`http://localhost:6600/friendrequest/friendrequest/${id}`, {
        status: action,
      })
      .then((result) => {
      ...
      >
        <Container maxWidth="xl">
          <Toolbar disableGutters>
            ...
            <button onClick={() => requestStatusHandler(user._id)}>
              ClickAction
            </button> */}
