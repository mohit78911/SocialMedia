...
export default function Feed({ user, handleClickOpen, handleClose, open }) {
  ...
  const handleLike = async (postId) => {
    await axios.post("http://localhost:6600/like/likepost",{postId : postId, userId : user._id}).then((res)=>{
      console.log("like")
    })
    try {
      // Send a POST request to the backend to like the post
      await axios.post("http://localhost:6600/like/likepost", {
        postId: postId,
        userId: user._id,
      });
      console.log("like_done");
      setIsLiked(true);
      // onLike(post._id, true);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const handleUnlike = async () => {
    try {
      // Send a DELETE request to the backend to remove the like
      await axios.delete(`/likes/like/${post.likeId}`);
      setIsLiked(false);
