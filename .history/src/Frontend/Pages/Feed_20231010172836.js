...
export default function Feed({
  ...
  const likeHandler = (val) => {
    const filterLike = likedata.filter((value) => value._id === val);

    console.log("likedata", filterLike);
    const likeData = {
      like: postLike,
      // userId: likedata.userId._id,
      // postId: likedata.postId._id,
    };
    const likeapi = axios.put(
      `http://localhost:6600/like/update/${val}`,
      likeData
    );

    if (val._id === filterLike) {
      setPostlike(true);
      likeapi
        .then((res) => {
