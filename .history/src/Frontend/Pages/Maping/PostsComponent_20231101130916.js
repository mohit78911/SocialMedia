...
export default function PostsComponent({
  ...
  const [lQuentity, setLQuentity] = useState([]);

  const QuentityHandler = (postid) => {
    axios
      .get(`http://localhost:6600/like/${postid}`, {
        headers: { authorization: token },
      })
      .then((result) => setLQuentity(result.data))
                ...
                <Box className="likebtnsContainer">
                  <Box
                    ...
                    onClick={() => {
                      ...
                      QuentityHandler(value._id);
                    }}
                  >
                    Likes {lQuentity.length[value._id]}
                  </Box>
                  <Box
                    sx={{ cursor: "pointer", opacity: "0.6" }}
                    onClick={() => {
