...
export default function PostsComponent({
  ...
  const [likesPostComp, setLikesPostComp] = useState([]);
  const getAllLikesDataHandler = (postid) => {
    axios
      .get("http://localhost:6600/likesdata/postid")
      .then((result) => {
        setLikesPostComp(result.data);
        console.log("likes, postComp", result.data);
      })
      .catch((error) => {
        console.log(error);
      });
