...
export default function PostsComponent({
  ...
  const getAllLikesData = () => {
    axios
      .get("http://localhost:6600/likesdata")
      .then((result) => {
        console.log("likesData", result.data);
  ...
  const filterLikesCount = allLikes.filter((item) => item.like === postId._id);
  console.log("filterLikesCount", filterLikesCount);
  return (
    <Box>
      <Box>
        {post.map((value, i) => {
